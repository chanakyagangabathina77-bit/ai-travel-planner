...
- `app/login/page.js`: login flow
- `app/register/page.js`: registration flow
- `app/dashboard/page.js`: saved trip overview
- `app/create-trip/page.js`: trip generation form
- `app/trip/[id]/page.js`: trip detail and editing screen

The frontend stores the JWT in `localStorage` and sends it through Axios interceptors in `services/api.js`.

### Backend

- `server.js`: Express bootstrap, middleware, and route registration
- `routes/authRoutes.js`: register and login endpoints
- `routes/tripRoutes.js`: trip CRUD and editing endpoints
- `controllers/authController.js`: account creation and sign-in
- `controllers/tripController.js`: trip creation, retrieval, deletion, and trip edits
- `services/geminiService.js`: AI itinerary generation and fallback generator
- `models/Trip.js`: MongoDB trip schema
